DayZ Chernarus Krasno NEAF Grenade / M79 / Explosive Practice Range For Console and PC json Custom Spawner Mod Instructions & Terms Of Use

This custom object spawner json file will spawn in a throwable practice range at the NEAF, with explosive and smoke grenades, m79's with ammo, claymores and the parts to make IEDs.
...
(Optional: For advanced users I have included the original DayZ Editor Mod file "editable-naderange.dze" which can be imported into DayZ Editor Mod on PC for customisation.)
...
Upload "nade-practicerange.json" from the extracted files to inside the "custom" folder of the mission directory on your server. This file places the practice range on your map.
...
Edit it to look like this: 
	...
	"objectSpawnersArr": ["custom/nade-practicerange.json"]
	
If you already are calling custom jsons to spawn items, seperate the files like this:

	"objectSpawnersArr": ["custom/nade-practicerange.json","custom/anotherfile.json","custom/differentfile.json"]
